...
export async function saveGift(g:BirthdayGift,coverFile:File|null,photoFiles:Map<string,File>){
 ...
 const{error:giftError}=await c.from('birthday_gifts').upsert({id:g.id,user_id:user.id,public_id:g.publicId,recipient_name:g.recipientName,creator_name:g.creatorName,event_type:g.eventType,theme_color:g.themeColor,birthday:g.birthday,cover_image:cover,letter:g.letter,gift:{...g.gift,customerService:g.creatorName},is_published:g.isPublished},{onConflict:'id'})
