...
export function fromDb(r: Row): BirthdayGift {
  ...
  return {
    ...
    contractTerms: (r.contract_terms ?? [])
      .sort((a: Row, b: Row) => a.sort_order - b.sort_order)
      .map((t: Row) => ({ id: t.id, content: t.content, sortOrder: t.sort_order })),
...
export async function getPublicGift(id: string) {
  const { data, error } = await client()
    ...
    .maybeSingle()
  if (error) fail('공개 Box 조회 실패', error)
...
async function upload(file: File, path: string) {
  ...
  if (error) fail('사진 업로드 실패', error)
  return c.storage.from('birthday-images').getPublicUrl(path).data.publicUrl
}
export async function saveGift(
  ...
  if (coverFile) {
    ...
    cover = await upload(coverFile, `${user.id}/${g.id}/cover-${crypto.randomUUID()}.${ext}`)
  }
  const { error: giftError } = await c.from('birthday_gifts').upsert(
    {
      id: g.id,
      user_id: user.id,
      ...
      theme_color: g.themeColor,
      birthday: g.birthday,
      cover_image: cover,
      letter: g.letter,
      gift: { ...g.gift, customerService: g.creatorName },
  ...
  for (const [i, p] of g.photos.entries()) {
    ...
    if (file) {
      const ext = file.name.split('.').pop()?.toLowerCase() || 'jpg',
        path = `${user.id}/${g.id}/photos/${p.id}-${crypto.randomUUID()}.${ext}`
      photos.push({
        id: p.id,
        gift_id: g.id,
  ...
  let result = await c.from('birthday_photos').delete().eq('gift_id', g.id)
  if (result.error) fail('기존 사진 정리 실패', result.error)
  if (photos.length) {
    result = await c.from('birthday_photos').insert(photos)
    if (result.error) fail('사진 정보 저장 실패', result.error)
  }
  result = await c.from('contract_terms').delete().eq('gift_id', g.id)
  if (result.error) fail('기존 계약 조항 정리 실패', result.error)
  if (g.contractTerms.length) {
    result = await c.from('contract_terms').insert(
      g.contractTerms.map((t, i) => ({
        id: t.id,
        gift_id: g.id,
        content: t.content,
        sort_order: i,
    ...
    if (result.error) fail('계약 조항 저장 실패', result.error)
  }
  return { ...g, coverImage: cover }
}
export async function deleteGift(g: BirthdayGift) {
  const c = client(),
    paths = g.photos.map(p => p.storagePath).filter((p): p is string => Boolean(p))
  if (paths.length) await c.storage.from('birthday-images').remove(paths)
  const { error } = await c.from('birthday_gifts').delete().eq('id', g.id)
  if (error) fail('Box 삭제 실패', error)
}
