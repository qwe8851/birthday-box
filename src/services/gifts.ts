...
export function fromDb(r:Row):BirthdayGift{return{id:r.id,publicId:r.public_id,recipientName:r.recipient_name,birthday:r.birthday,coverImage:r.cover_image??DEFAULT_COVER_IMAGE,letter:r.letter??'',gift:r.gift,photos:(r.birthday_photos??[]).sort((a:Row,b:Row)=>a.sort_order-b.sort_order).map((p:Row)=>({id:p.id,url:p.url,sortOrder:p.sort_order,storagePath:p.storage_path})),contractTerms:(r.contract_terms??[]).sort((a:Row,b:Row)=>a.sort_order-b.sort_order).map((t:Row)=>({id:t.id,content:t.content,sortOrder:t.sort_order})),isPublished:r.is_published,createdAt:r.created_at,updatedAt:r.updated_at}}
...
export async function saveGift(g:BirthdayGift,coverFile:File|null,photoFiles:Map<string,File>){
 ...
 const photos=[]
 for(const[i,p]of g.photos.entries()){const file=photoFiles.get(p.id);if(file){const ext=file.name.split('.').pop()?.toLowerCase()||'jpg',path=`${user.id}/${g.id}/photos/${p.id}.${ext}`;photos.push({id:p.id,gift_id:g.id,url:await upload(file,path),storage_path:path,sort_order:i})}else if(p.storagePath)photos.push({id:p.id,gift_id:g.id,url:p.url,storage_path:p.storagePath,sort_order:i})}
